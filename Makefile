...
COMM_SRCS = $(comdir)pure2.cpp $(comdir)inquire.cpp $(comdir)filesystem.cpp $(comdir)c.cpp $(comdir)common.c $(comdir)compiler.cpp $(comdir)cpu.cpp $(comdir)cygwin.c $(comdir)exepath.cpp $(comdir)env.cpp $(comdir).cpp $(comdir)libpath.cpp $(comdir)limits.cpp $(comdir)mkdtemp.c $(comdir)owner.c $(comdir)os.c $(comdir)partition.c $(comdir)realpath.cpp $(comdir)space.cpp $(comdir)sysctl.cpp $(comdir)symlink.cpp $(comdir)touch.cpp $(comdir)uid.cpp $(comdir)uname.cpp $(comdir)which.c $(comdir)windows.cpp $(comdir)winsock.cpp

ifeq (cpp,1)
SRCS = $(comdir)c_ifc.cpp $(comdir)copy.cpp $(comdir)ifc.cpp $(comdir)mkdir.cpp $(comdir)mkdtemp.cpp $(comdir)pure.cpp $(comdir)platform.cpp $(comdir)resolve.cpp $(comdir)time.cpp
else
SRCS = $(cdir)copy.c $(cdir)mkdir.c $(cdir)mkdtemp.c $(cdir)pure.c $(cdir)platform.c $(cdir)resolve.c $(cdir)time.c
endif
